...
station_lonlat = mutate_geocode(station_df, station_list, source="google") #mutate_geocode 다수의 위도, 경도 처리함. 
head(station_lonlat)
class(station_lonlat)

# -- ggmap::qmap : 지도 그리기(google)
qmap('seoul', zoom = 11) #zoom 2~20까지 설정 가능. 18:빌딩, 세계지도:2정도. 도시:10~13정도.
qmap('seoul', zoom = 11, color='bw') #blackwhite
...
seoul_lonlat = unlist(geocode('seoul', source='google')) #unlist 해서 벡터로 만든다
class(seoul_lonlat)
names(seoul_lonlat)
...
wifi_data[,2]
# for(i in 1:14){...}              dim(wifi_data)[2] : 열 
...
if (!require(leaflet)) install.packages("leaflet") #leaflet이 구글보다 더 상세?


leaflet(wifi_data) %>%
  setView(lng = seoul_lonlat[1],
          lat = seoul_lonlat[2], #시청을 가리킴
          zoom = 11) %>%
...
leaflet(wifi_data) %>%
  ...
  addCircles(lng = ~ lon, lat = ~lat, popup = ~telecom) #파란점에 마우스를 클릭하면 이동통신사들이 나옴. 

## - telecom 마다 다른 색을 입히자.
if (!require(graphics)) install.packages("graphics")

rcols <- rainbow(length(unique(wifi_data$telecom)), alpha = NULL) #통신사가 3개이기 때문에 7가지 색 중 3가지 색이 나옴. 알파는 투명도 ! 
#rcols 는 세개의 문자열을 가지고 있는 벡터.
