#### 13-1 기초 통계
## 대표값과 산포(변동성->분산,범위,표준편차,중간값에 대응되는 IQR,표준오차. 표준편차를 평균으로 나눈 것. )
x <- runif(50, 1, 100)
x
hist(x)
y <- c(x, 1000)
...
## 이항분포
# - tv리모콘 문제: 채널 50에서 6번 눌렀을 때 채널 50. 위세번, 아래 세번 누름. 
# - 6C3 (1/2)^6
6*5*4/(3*2)*(0.5^6)
dbinom(3, 6, 0.5)
#채널을 50에서 6번 눌렀을 때 채널이 50 이하일 확률. 다운버튼만 6번 or 5번or 4번 or3번 눌렀을 경우. ?
sum=0
for(i in 0:3){
  sum = sum +dbinom(3,6,0.5)
}
sum

pbinom(3, 6, 0.5)

#채널이 51 이하일 확률 다운버튼을 적어도 4번은 눌러야함. 
pbinom(4,6,0.5)
