...
mpg2 %>% group_by(drv) %>% summarise(m = mean(hwy)) %>% arrange(desc(m))
...
mpg2<-mpg %>% filter(!is.na(hwy)) %>% group_by(drv) %>% summarise(m = mean(hwy))
mpg2

# • Q3. drv 그룹별 hwy의 평균으로 결측치를 대체하고자 한다면?

mpg3<- mpg %>% summarise(mean_hwy = mean(hwy, na.rm = T))
