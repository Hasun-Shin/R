...
install.packages("foreign")  # foreign 패키지 설치 ctrl shif c
...
sexsymbol <- c("male", "female")
welfare$sex <- factor(welfare$sex, levels = sexsymbol)

str(welfare$sex)


# welfare <- rename(welfare, factor=sex)
# welfare$factor <- ifelse(welfare$factor == "male", 1, 2)
# 
# welfare
...
sum(welfare$income == 9999, na.rm=T) #9999는 결측치. 잘 삭제되었는지 확인함. 
...
welfare_age <-welfare %>% filter(age>=20 & age<=60)

pop<-dim(welfare_age)[1]
incomeX<- table(is.na(welfare_age$income))[2]


age_incomeX <- welfare_age %>% select(age, income) %>% 
  summarise(ratio = incomeX /pop)  
...
welfare %>% filter(age>=20 & age<60) %>% 
  mutate(unempness = ifelse(is.na(income), 'yes','no'),
         age10 = age %>%10*10) %>% 
  select(sex,age10,unempness) %>% 
  group_by(sex, age10, unempness) %>% 
  summarise(n=n()) %>% 
  mutate(rate=n/sum(n))


# - 20살에서 60살까지의 연령대에 대해 이에 대한 빈도수를 계산해보자.
age2060<-welfare %>% filter(age>=20 & age<=60)
qplot(age2060$age)
table(age2060$age)

ggplot(data = age2060, aes(x = age)) + geom_histogram(bins=5)

ggplot(age2060, aes(x = age)) + geom_bar()

# Quiz: income의 결측치를 0으로 평가할 때, 
# 성별 income의 평균을 비교하라.

welfare_income0<- welfare %>% ifelse(is.na(income),0,income)

welfare$income <- ifelse(is.na(welfare$income),0,welfare$income)

welfare %>% filter(age>=20& age<=60) %>% group_by(sex) %>% summarise(m = mean(income))
welfare %>%  group_by(sex) %>% summarise(m = mean(income))


dim(welfare)

#quiz: 20대 30대 40대 50대 60대 이상으로 분류해보시오. 임금 평균. 
