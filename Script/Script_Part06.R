...
exam %>% filter(!(class %in% c(3, 1))) #class 가 3과 1에 속하지 않은 것. 
...
## python의 eval(파이썬 코드로 만듬), exec(실행함) 함수와 유사한 기능을 
## parse <=> eval(파이썬) , eval(R) <=> exec(파이썬)  

parse(text="class != 3") # 해당 텍스트를 parse 함 -> expression 이 만들어짐. 
eval(parse(text="2+3"))
...
class1mean <- mean(class1$hwy)
class2mean<- mean(class2$hwy)
...
audimean <- mean(audi$cty)
toyotamean<- mean(toyota$cty)
...
cars <- mpg %>% filter(manufacturer=="chevrolet"|manufacturer=="ford"|manufacturer=="honda")
cars2 <- mpg %>% filter(manufacturer %in% c("chevrolet","ford","honda"))
...
suvmean <- mean(suv$cty)
compactmean<- mean(compact$cty)
...
sort(exam$math)
math_idx<-sort(exam$math, index.return=T)$ix #ix 정렬시켜야 하는 인덱스. 
...
mpg2<-mpg %>% filter(manufacturer=="audi") %>% group_by(model) %>% arrange(desc(hwy))

head(mpg2)

mpg2<-mpg %>% filter(manufacturer=="audi") %>% group_by(model) %>% summarise(mh=mean(hwy)) %>% 
  arrange(desc(mh)) %>% 
  head(5)
...
mpg2 %>% mutate(mhc = (hwy+cty)/2) %>% group_by(model) %>% summarize(mmhc = mean(mhc)) %>% arrange(desc(mmhc)) %>% head(3)
...
### 퀴즈. 각 반별로 science 의 최고점과 최저점인 학생 id를 구하라.
...
class3 <- df_exam %>% filter(class==3)
class4 <- df_exam %>% filter(class==4)


which.max(df_exam$science)
which.min(df_exam$science)

a<-class1[(class1$science==max(class1$science)),1:2]
aa<-class1[(class1$science==min(class1$science)),1:2]

b<-class2[(class2$science==max(class2$science)),1:2]
bb<-class2[(class2$science==min(class2$science)),1:2]

c<-class3[(class3$science==max(class3$science)),1:2]
cc<-class3[(class3$science==min(class3$science)),1:2]

d<-class4[(class4$science==max(class4$science)),1:2]
dd<-class4[(class4$science==min(class4$science)),1:2]

bind_rows(a,aa,b,bb,c,cc,d,dd)
...
df_exam[which.min(df_exam$science),1]
...
### Quiz 실습하기 p,150###
mpg %>% group_by(class) %>% summarise(mcty = mean(cty)) %>% arrange(desc(mcty))


mpg %>% group_by(manufacturer) %>% summarise(mhwy = mean(hwy)) %>% arrange(desc(mhwy)) %>% head(3)

mpg2<-mpg %>% filter(class=="compact") %>% group_by(manufacturer) %>% summarise(freq = n())
...
# cbind, rbind, merge :r base 함수들 
# dplyr 패키지의 bind_cols, bind_rows, inner_join, left_join, ... -> 이 함수들을 사용하는 것이 더 빠름. 
example(merge)
...
midwest2 <- midwest %>% mutate(pp = ((poptotal-popadults)/poptotal)*100)

midwest3<-midwest %>% mutate(pp = ((poptotal-popadults)/poptotal)*100) %>% group_by(county) %>% summarise(mean_pp = mean(pp)) %>% arrange(desc(mean_pp)) %>% head(5)

midwest3


midwest4<-midwest2 %>% mutate(grade = ifelse(midwest2$pp >= 40, "large", ifelse(midwest2$pp >=30 |pp<40, "middle","small")))

midwest4$grade

midwest %>% mutate(asian_rate = popasian*100/poptotal) %>% 
